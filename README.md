...
- **Agent 写权限收敛**：ACP 客户端只允许读取当前活动文档，拒绝 agent 侧直接写文件。
...
│ acp-client.js: ACP JSON-RPC client + Codex CLI fallback                  │
└──────────────────────────── ACP / Codex CLI ─────────────────────────────┘
                                      │
...
| ACP client | `server/lib/acp-client.js` | ACP session、prompt、权限、fallback |
...
当前实现是 **document-level ACP session**：

- 一个活动文档对应一个 `AcpDocumentAgent`。
- 第一次向 Codex 提问时创建 ACP session。
- 同一文档内的多个 thread 共享这个 session。
- 请求通过 `promptLock` 串行化。
- 切换文档时销毁旧 agent，并为新文档创建新的 agent/session。
...
玄鸟解析该 replacement 后由服务端写入文档。Codex 本身不会获得直接写文件权限。

如果 ACP 不可用，且没有显式设置 `XUANNIAO_ACP_CMD`，玄鸟会 fallback 到本地 Codex CLI 的 `codex exec`。
...
- Node.js >= 20
- npm
- 可选：ACP adapter `codex-acp`
- 可选：Codex CLI `codex`，用于 fallback
...
```bash
XUANNIAO_DISABLE_CODEX_FALLBACK=1 npm start -- prd.md
```
...
- 当前 session 粒度是 document-level，不是 thread-level；同一文档内的 thread 会共享 ACP 上下文。
- Thread 持久化使用 JSON sidecar，适合 MVP，后续可以迁移到 SQLite。
- 文档修改目前使用选区 replacement，不是完整 patch review flow。
- Agent 权限被收敛为读取当前文档，复杂跨文件任务需要后续扩展权限模型。
...
- **Constrained agent writes**: the ACP client lets the agent read only the active document and denies agent-side mutations.
...
| ACP client | `server/lib/acp-client.js` | ACP session, prompt construction, permissions, fallback |
...
- Each thread stores selected text, anchor positions, line numbers, and message history.
...
Xuanniao currently uses a **document-level ACP session**:

- One active document owns one `AcpDocumentAgent`.
- The ACP session is created lazily on the first prompt.
- Multiple threads in the same document share the same session.
- Prompts are serialized with `promptLock`.
- Switching documents disposes the old agent and creates a new one.
...
- current document path and title
- selected text
- anchor JSON
...
Xuanniao parses and applies that replacement itself. Codex does not receive unrestricted filesystem write permission.

If ACP is unavailable and `XUANNIAO_ACP_CMD` is not explicitly set, Xuanniao falls back to `codex exec`.
...
- Node.js >= 20
- npm
- optional: an ACP adapter named `codex-acp`
- optional: Codex CLI for fallback
...
- ACP session scope is document-level, not thread-level.
...
- Agent permissions are intentionally constrained to the active document.
